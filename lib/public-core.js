...
const PUBLIC_APPLICATIONS_PUSH_TIMEOUT_MS = Number(process.env.PUBLIC_APPLICATIONS_PUSH_TIMEOUT_MS || 20000);
...
function publicJobSelectColumns(supportsJobSource = false) {
  return supportsJobSource
    ? `${PUBLIC_JOB_SELECT_BASE},job_source`
    : PUBLIC_JOB_SELECT_BASE;
}

let supabaseClient = null;
let bucketPromise = null;
let jobSourceColumnSupportPromise = null;
...
async function supportsJobSourceColumn() {
  if (!jobSourceColumnSupportPromise) {
    jobSourceColumnSupportPromise = (async () => {
      ...
      if (message.includes('job_source') || message.includes('column') || message.includes('schema cache')) {
        console.warn('job_source column is not available yet; falling back to updated_at ordering');
        return false;
      }
      throw new Error(`Failed to inspect public jobs schema: ${error.message}`);
    })().catch((error) => {
      jobSourceColumnSupportPromise = null;
      throw error;
    });
  }
  return jobSourceColumnSupportPromise;
}
...
function getParamList(value) {
  if (Array.isArray(value)) {
    return value.flatMap(getParamList);
  }
  ...
  if (raw[0] === '[') {
    try {
      ...
      if (Array.isArray(parsed)) return parsed.flatMap(getParamList);
    } catch (error) {
      // Fall through and treat it as a single literal filter value.
    }
  }
...
function getIntegerParam(value, fallback, { min = 0, max = Number.MAX_SAFE_INTEGER } = {}) {
...
function splitFilterOptionField(value) {
  if (value == null) return [];

  if (Array.isArray(value)) {
    return value.flatMap(splitFilterOptionField);
  }

  if (typeof value === 'object') {
    const objectValue = value.name || value.label || value.value || value.title || '';
    return objectValue ? splitFilterOptionField(objectValue) : [];
  }
  ...
  if (['[', '{', '"'].includes(raw[0])) {
    try {
      return splitFilterOptionField(JSON.parse(raw));
    } catch (error) {
      // Fall through to delimiter splitting for malformed stored text.
    }
  }
...
function normalizeJobRow(row) {
  return {
    ...
    job_source: normalizeText(row.job_source),
...
function toPostgrestIlikePattern(value) {
  return `*${String(value || '').replace(/[*,()]/g, ' ').replace(/\s+/g, ' ').trim()}*`;
}

function toSqlIlikePattern(value) {
  return `%${String(value || '').replace(/[\\%_]/g, '\\$&')}%`;
}

function applyAnyTextFilter(query, column, values) {
  const terms = getParamList(values).map((value) => value.trim()).filter(Boolean);
  if (!terms.length) return query;
  if (terms.length === 1) return query.ilike(column, toSqlIlikePattern(terms[0]));

  const clauses = terms
    .map((term) => toPostgrestIlikePattern(term))
    .filter((pattern) => pattern !== '**')
    .map((pattern) => `${column}.ilike.${pattern}`);

  return clauses.length ? query.or(clauses.join(',')) : query;
}

function applyJobFilters(query, { category, location, search }) {
  let nextQuery = applyAnyTextFilter(query, 'category', category);
  nextQuery = applyAnyTextFilter(nextQuery, 'location', location);

  const searchText = String(search || '').trim();
  if (searchText) {
    const pattern = toPostgrestIlikePattern(searchText);
    ...
    nextQuery = nextQuery.or(searchableColumns.map((column) => `${column}.ilike.${pattern}`).join(','));
  }

  return nextQuery;
}

function buildJobsQuery({ category, location, search, withCount = false, supportsJobSource = false }) {
  const table = getSupabaseAdmin().from(JOBS_TABLE);
  const selectColumns = publicJobSelectColumns(supportsJobSource);
  let query = withCount ? table.select(selectColumns, { count: 'exact' }) : table.select(selectColumns);
  if (supportsJobSource) {
    query = query.order('job_source', { ascending: true, nullsFirst: false });
  }
  query = query.order('updated_at', { ascending: false });

  return applyJobFilters(query, { category, location, search });
}

async function listJobs({ category, location, search, limit = null, offset = 0 }) {
  const supportsJobSource = await supportsJobSourceColumn();
...
async function getJobByNumber(jobNumber) {
  ...
  const { data, error } = await getSupabaseAdmin()
    .from(JOBS_TABLE)
    .select(publicJobSelectColumns(supportsJobSource))
...
async function listDistinctValues(columnName) {
  ...
  for (const row of data || []) {
    for (const value of splitFilterOptionField(row?.[columnName])) {
      const key = value.toLocaleLowerCase('he-IL');
      if (!values.has(key)) values.set(key, value);
    }
  }
  return Array.from(values.values()).sort((a, b) => a.localeCompare(b, 'he-IL'));
}
...
async function syncJobs(upserts, deletedJobNumbers, syncedAtRaw) {
  ...
  if (normalizedRows.length) {
    ...
    const hydratedRows = normalizedRows.map((row) => {
      ...
      if (!supportsJobSource) delete hydrated.job_source;
      return hydrated;
    });
...
async function createApplicationFromRequest(req) {
  ...
  try {
    ...
    let localPush = { sent: false, skipped: true };
    try {
      localPush = await pushApplicationToLocal(row, uploadResult);
      if (localPush.success) {
        try {
          const deleteResult = await deleteSyncApplications([applicationUuid]);
          localPush.deleted_public_backup = Number(deleteResult?.deleted || 0);
        } catch (deleteError) {
          localPush.delete_error = deleteError.message;
        }
      }
    } catch (pushError) {
      localPush = { sent: true, success: false, error: pushError.message };
    }
...
function getLocalApplicationsPushUrl() {
  if (process.env.PUBLIC_APPLICATIONS_PUSH_URL) {
    return process.env.PUBLIC_APPLICATIONS_PUSH_URL;
  }
  if (process.env.LOCAL_APPLICATIONS_PUSH_URL) {
    return process.env.LOCAL_APPLICATIONS_PUSH_URL;
  }

  const publicBaseUrl = String(process.env.NEXUS_PDF_PUBLIC_BASE_URL || '').trim().replace(/\/+$/, '');
  if (!publicBaseUrl) return '';

  try {
    const parsed = new URL(publicBaseUrl);
    parsed.pathname = '/api/sync/applications/push';
    parsed.search = '';
...
async function pushApplicationToLocal(application, uploadResult) {
  const pushUrl = getLocalApplicationsPushUrl();
  if (!pushUrl) {
    return { sent: false, skipped: true, reason: 'PUBLIC_APPLICATIONS_PUSH_URL is not configured' };
  }
  if (!PUBLIC_SYNC_SECRET) {
    return { sent: false, skipped: true, reason: 'PUBLIC_SYNC_SECRET is not configured' };
  }
  ...
  const payload = {
    ...
    cv_file_type: application.cv_file_type,
    noresume: application.noresume,
    created_at: application.created_at,
  };

  if (!normalizeBoolean(application.noresume)) {
    if (!uploadResult?.pdfBuffer?.length) {
      throw new Error('Application resume file is missing and cannot be pushed');
    }
    payload.cv_file_base64 = uploadResult.pdfBuffer.toString('base64');
  }

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), PUBLIC_APPLICATIONS_PUSH_TIMEOUT_MS);
  try {
    const response = await fetch(pushUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-sync-secret': PUBLIC_SYNC_SECRET,
      },
    ...
    if (!response.ok || !data?.success) {
      throw new Error(data?.message || `Local application push failed (${response.status})`);
    }

    return { sent: true, success: true, ...data };
  } finally {
...
async function handleGetJobs(req, res) {
  ...
  try {
    const limitRaw = getParam(req.query.limit);
    const limit = limitRaw === ''
      ? null
      : getIntegerParam(req.query.limit, 30, { min: 1, max: 100 });
    const result = await listJobs({
      category: getParamList(req.query.category),
      location: getParamList(req.query.location),
      search: getParam(req.query.search),
      limit,
      offset: getIntegerParam(req.query.offset, 0, { min: 0 }),
    });
    return json(res, 200, {
      success: true,
      count: result.total,
      ...
      supports_job_source: Boolean(result.supportsJobSource),
      data: result.rows,
    });
  } catch (error) {
...
async function handleGetJobByNumber(req, res) {
  ...
  try {
    ...
    if (!row) {
      return json(res, 404, {
        success: false,
        code: 'JOB_INACTIVE_OR_NOT_FOUND',
        message: 'המשרה כבר לא פעילה או הוסרה מהאתר',
      });
    }
