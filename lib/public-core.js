...
function buildResumeStoragePath(fullName, originalName) {
  ...
  const base = safeBaseName(fullName || path.basename(originalName || 'resume.pdf', path.extname(originalName || 'resume.pdf'))).slice(0, 80);
...
async function convertDocBufferToPdf(fileBuffer, originalFilename) {
  if (!DOC_PDF_CONVERTER_URL) {
    throw new Error('DOC files require DOC_PDF_CONVERTER_URL to convert to PDF on Vercel.');
  }
...
async function syncJobs(upserts, deletedJobNumbers) {
  ...
  if (normalizedRows.length) {
    const { error } = await getSupabaseAdmin()
      .from(JOBS_TABLE)
      .upsert(normalizedRows, { onConflict: 'job_number' });
    if (error) {
...
async function handleSyncJobs(req, res) {
  ...
  try {
    const result = await syncJobs(upserts, deletedJobNumbers);
    return json(res, 200, {
      success: true,
